...
interface Props {
  identity: VenueIdentity & { venue: Venue }
  isPrimary: boolean
  onRename: (venueId: string, newHandle: string) => Promise<void>
  onSetPrimary: (venueId: string) => Promise<void>
}

export function VenueIdentityRow({
  identity,
  isPrimary,
  onRename,
  onSetPrimary,
  ...
  const [handle, setHandle] = useState(identity.venue_handle)
  ...
  const handleRename = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        await onRename(identity.venue_id, trimmed)
        setEditing(false)
      } catch (err: unknown) {
  ...
  const handleSetPrimary = () => {
    ...
    startTransition(async () => {
      try {
        await onSetPrimary(identity.venue_id)
      } catch (err: unknown) {
  ...
  return (
    <div style={{ padding: '0.75rem 0', borderBottom: '1px solid #eee' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
        <div style={{ flex: 1 }}>
          <strong>{identity.venue.name}</strong>
          {isPrimary && (
          ...
          <div style={{ color: '#555', fontSize: '0.9rem', marginTop: '0.15rem' }}>
            Handle: <strong>{identity.venue_handle}</strong>
          </div>
        </div>
