...
const BENEFITS = [
  ...
  {
    title: 'Email & SMS replies, notifications & reminders',
    body: 'They can reply by email/SMS and receive updates.',
  },
  {
    title: 'Private by default',
    body: 'Contact details stay hidden unless you share.',
  },
]
...
export function AddMyContactPanel({
  ...
  useEffect(() => {
    ...
    const syncBodyScroll = () => {
      if (media.matches && originalOverflow === null) {
        originalOverflow = document.body.style.overflow
        document.body.style.overflow = 'hidden'
      } else if (!media.matches && originalOverflow !== null) {
  ...
  const shellClass = mode === 'modal'
    ? 'fixed inset-0 z-50 flex items-end justify-center bg-[#0B1F44]/35 p-0 sm:items-center sm:p-4'
    : 'fixed inset-0 z-50 flex items-end justify-center bg-[#0B1F44]/35 p-0 sm:static sm:block sm:bg-transparent sm:p-0'
  const panelClass = [
    'relative flex h-[100dvh] max-h-[100dvh] w-full flex-col overflow-y-auto overflow-x-hidden bg-white px-4 pb-[calc(env(safe-area-inset-bottom)+1rem)] pt-5 shadow-[0_32px_80px_-32px_rgba(11,31,68,0.5)]',
    mode === 'modal'
      ? 'sm:h-auto sm:max-h-[90vh] sm:max-w-6xl sm:rounded-[36px] sm:border sm:border-[#D7E2F0] sm:px-8 sm:py-7 lg:px-10'
      : 'sm:h-auto sm:max-h-none sm:rounded-[40px] sm:border sm:border-[#E2E8F0] sm:px-8 sm:py-7 sm:shadow-[0_26px_70px_-42px_rgba(11,31,68,0.35)] lg:px-10',
  ].join(' ')
  ...
  return (
    <div className={shellClass}>
      <button
        ...
        onClick={onClose}
        className={mode === 'modal' ? 'absolute inset-0' : 'absolute inset-0 sm:hidden'}
      />
      <section
      ...
      >
        <div className="sticky top-0 z-20 -mx-4 flex items-center justify-between gap-4 border-b border-[#E2E8F0] bg-white px-4 pb-4 sm:static sm:mx-0 sm:border-b-0 sm:px-0 sm:pb-0">
