-- Migration: Unified restart participation logic (override live RPCs)
-- v1.3 simplified semantics:
--   ANY removed user can "Request to Join" again (scope only for first entry)
--   ORG can "Invite" any removed user again (ORG-only)
--   Nominate handles re-nomination of removed users (UPDATE instead of INSERT)
...
-- =====================================================================
-- 0) Safety: ensure required tables/functions exist (optional but useful)
-- =====================================================================
-- Uncomment if you want hard fails when schema is unexpected
-- DO $$
-- BEGIN
--   IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname='matches') THEN
--     RAISE EXCEPTION 'missing table: matches';
--   END IF;
--   IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname='match_participants') THEN
--     RAISE EXCEPTION 'missing table: match_participants';
--   END IF;
--   IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname='match_participant_reconcile_status') THEN
--     RAISE EXCEPTION 'missing function: match_participant_reconcile_status';
--   END IF;
-- END $$;
...
COMMENT ON FUNCTION public.rpc_match_request_join(uuid) IS
  'v1.3: User requests to join. Re-request allowed for any removed user (no scope check). First entry requires scope. Clears removed_* on restart.';
...
COMMENT ON FUNCTION public.rpc_match_nominate_user(uuid, uuid) IS
  'v1.3: Nominate user. Re-nomination allowed for any removed user. join_method=requested, nominated_by=caller. Requires user accept + ORG approve. Clears removed_* on restart.';
