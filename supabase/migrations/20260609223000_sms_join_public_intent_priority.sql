-- Hotfix: JOIN belongs to the public join SMS flow.
-- Do not let active direct invitation reply codes intercept a public join
-- recipient who follows the SMS instruction: "Reply JOIN to request a spot."
...
begin
  if v_phone is null then
    return 'We could not read your phone number. Reply YES with your invite code, NO with your invite code, OUT with your code if you need to back out, DETAILS with your code for the match link, or JOIN for a public join text.';
  end if;
  ...
  if v_body in ('HELP', '?') then
    return 'Reply YES with your invite code, NO with your invite code, OUT with your code if you need to back out, DETAILS with your code for the match link, or JOIN for a public join text.';
  end if;

  if v_body in ('MAYBE', 'M') then
    return 'Maybe is not supported yet. Reply YES with your invite code or NO with your invite code for a pending invite, JOIN for a public join text, or OUT with your code if you need to back out.';
  end if;
  ...
  if v_code is not null then
    ...
    if not found then
      return 'We could not find that invite code. Reply YES with your invite code, NO with your invite code, or DETAILS with your invite code for the match link.';
    end if;
    ...
    if coalesce(v_candidate_count, 0) = 0 then
      ...
      if v_action = 'withdraw' then
        return 'We could not find a match you can back out of for this number. Reply YES with your invite code or NO with your invite code for a pending invite.';
      end if;
