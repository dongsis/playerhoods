-- Normalize new Contact Player host invites to direct invited semantics.
...
comment on function public.rpc_match_nominate_guest(uuid, uuid) is
  'Compatibility Contact Player admission RPC. Organizer-created Contact Player rows are direct invites; non-organizer rows remain nominations. Suppresses opted-out contact communication channels before creating invitation send events.';
...
comment on function public.rpc_match_invite_contact_person(uuid, uuid) is
  'P4 person_id-first Contact Player match invite wrapper. Requires the caller to own or explicitly save a private contact scope, resolves registered-user or private contact channel internally, and writes organizer Contact Player rows as direct invites while non-organizer rows remain nominations.';
