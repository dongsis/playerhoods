alter table public.profiles
  ...
  add column if not exists responsible_use_version text;

update public.profiles
set onboarding_profile_completed = onboarding_completed
where onboarding_profile_completed = false
  and onboarding_completed = true;

comment on column public.profiles.onboarding_profile_completed is
  'True after the required basic onboarding profile data is saved successfully, before final legal confirmation.';

comment on column public.profiles.age_confirmation_version is
  'Version string for the legal age confirmation agreed to during onboarding.';

comment on column public.profiles.terms_version is
  'Version string for the Terms of Use accepted during onboarding.';

comment on column public.profiles.privacy_version is
  'Version string for the Privacy Notice accepted during onboarding.';

comment on column public.profiles.responsible_use_version is
  'Version string for the responsible-use confirmation accepted during onboarding.';
