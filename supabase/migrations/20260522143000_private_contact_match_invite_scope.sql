-- Match contact invite privacy hardening.
-- Contact Player invite candidates and invite delivery must be scoped to the
-- caller's own private contact channel. Shared group membership can expose a
-- person node for context, but it must not expose or use another member's
-- Contact Player invite channel.

create or replace function public.can_user_have_private_contact_person_scope(
  p_user_id uuid,
  p_person_id uuid
)
returns boolean
...
begin
  ...
  or exists (
    ...
    join public.guests g
     ...
     and g.status = 'active'
    where urg.owner_user_id = p_user_id
  ...
  or exists (
    ...
    where cis.recipient_user_id = p_user_id
...
comment on function public.can_user_have_private_contact_person_scope(uuid, uuid) is
  'Returns true only when the user owns or explicitly saved a private Contact Player scope for the person. This is stricter than contact-person exposure and is required before match contact invites may surface or use contact channels.';

grant execute on function public.can_user_have_private_contact_person_scope(uuid, uuid) to authenticated, service_role;
...
begin
  ...
  exposed_people as (
    ...
    where pr.actor_user_id = v_uid
      ...
      and public.can_user_have_private_contact_person_scope(v_uid, pr.person_id)
  ...
  eligible as (
    select
      ...
      exists (
        ...
        where pgc.person_id = r.person_id
          and (
            (
...
comment on function public.rpc_match_contact_person_targets(uuid, text) is
  'P4 person-level Contact Player invite candidates for match UI. Returns only the caller-owned or explicitly saved private contact scopes; no guest_id, contact_record_id, phone, email, or private channel fields.';
...
comment on function public.rpc_match_invite_contact_person(uuid, uuid) is
  'P4 person_id-first Contact Player match invite wrapper. Requires the caller to own or explicitly save a private contact scope, resolves registered-user or private contact channel internally, and does not expose guest/contact/channel identifiers to the caller.';
