...
AS $$
  WITH relevant_match_ids AS (
    ...
    WHERE m.status = 'active'
      AND public.is_caller_in_match_scope(m.id)
  )
  SELECT m.*
  FROM public.matches m
...
IS 'Returns all matches relevant to auth.uid() for dashboard/main list use: organizer matches, participant matches, identity-linked guest participant matches, notification-linked matches, and active matches visible via request scope. SECURITY DEFINER so dashboard list stays stable even when RLS visibility is narrower than inbox notification visibility.';
