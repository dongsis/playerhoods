...
BEGIN
  ...
  IF v_mp.guest_id IS NOT NULL THEN
    v_is_ready := v_mp.org_approved_at IS NOT NULL;
  ELSIF v_mp.user_id IS NOT NULL THEN
...
ALTER FUNCTION public.match_participant_reconcile_status(uuid) OWNER TO postgres;

COMMENT ON FUNCTION public.match_participant_reconcile_status(uuid)
IS 'Reconciles participant readiness into pending / confirmed / waiting_list / removed. waiting_list preserves a ready participant who does not currently have an active roster spot.';

CREATE OR REPLACE VIEW public.match_counts AS
SELECT
  m.id AS match_id,
  m.required_count,
  COUNT(mp.id) FILTER (WHERE mp.removed_at IS NULL AND mp.status::text = 'confirmed')::integer AS confirmed_count,
  COUNT(mp.id) FILTER (WHERE mp.removed_at IS NULL AND mp.status::text = 'pending')::integer AS pending_count,
  COUNT(mp.id) FILTER (WHERE mp.removed_at IS NULL AND mp.status::text = 'waiting_list')::integer AS waiting_count
...
GRANT ALL ON FUNCTION public.rpc_match_rebalance_roster(uuid) TO authenticated;
GRANT ALL ON FUNCTION public.rpc_match_rebalance_roster(uuid) TO service_role;
