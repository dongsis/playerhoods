...
) is 'Canonical profile update RPC. Uses text storage for shared_group_join_preference and availability_status while preserving discovery scope settings; availability_until remains date-typed; empty first/last names remain empty strings for NOT NULL profile columns.';
