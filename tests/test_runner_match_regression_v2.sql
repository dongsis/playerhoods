...
-- Covers: admission / nominate / proxy confirm / approve / remove / withdraw /
...
DECLARE
  ...
  LEO_UID      uuid := '8c4d0a92-7f5e-4321-a2bc-6de6b47f28f1'; -- Leo
...
BEGIN
  ...
  -- D04 Self-withdraw triggers waiting-list promotion without a second client rebalance call
  BEGIN
    INSERT INTO public.matches (
      organizer_id, status, venue_id, court_ids,
      match_date, start_time, duration_minutes,
      ...
      ORG_UID, 'active', CLUB_ID, '{}'::uuid[],
      current_date, '12:22'::time, 90,
      'tr_v2_D04_withdraw_promotes_waitlist', 1,
      ARRAY[SCOPE_GID]::uuid[],
      true, true, true, now()
    )
    RETURNING id INTO v_mid;
    ...
    SELECT * INTO v_mp
    FROM public.match_participants
    WHERE public.match_participants.match_id = v_mid AND public.match_participants.user_id = REAL_UID
    ORDER BY created_at DESC
    LIMIT 1;
    PERFORM public.rpc_match_org_approve_participant(v_mp.id);
    PERFORM public.rpc_match_rebalance_roster(v_mid);

    SELECT * INTO v_mp FROM public.rpc_match_admit_user(v_mid, P_UID);
    ...
    SELECT * INTO v_mp
    FROM public.match_participants
    WHERE public.match_participants.match_id = v_mid AND public.match_participants.user_id = P_UID
    ORDER BY created_at DESC
    LIMIT 1;
    PERFORM public.rpc_match_org_approve_participant(v_mp.id);
    PERFORM public.rpc_match_rebalance_roster(v_mid);
    ...
    WHERE mp.match_id = v_mid
      AND mp.user_id = P_UID
      AND mp.removed_at IS NULL
      AND mp.status::text = 'confirmed';

    IF v_mp.removed_at IS NOT NULL AND v_cnt = 1 THEN
      INSERT INTO _v2_results VALUES ('D04 Self-withdraw promotes waiting list', true, 'ok', v_mid);
    ELSE
      INSERT INTO _v2_results VALUES ('D04 Self-withdraw promotes waiting list', false, 'removed_at='||coalesce(v_mp.removed_at::text,'NULL')||' promoted_count='||v_cnt::text, v_mid);
    END IF;
  EXCEPTION WHEN OTHERS THEN
    INSERT INTO _v2_results VALUES ('D04 Self-withdraw promotes waiting list', false, 'exception: '||SQLERRM, v_mid);
  END;

  -- D05 Organizer remove also triggers waiting-list promotion without a second client rebalance call
  BEGIN
    INSERT INTO public.matches (
      organizer_id, status, venue_id, court_ids,
      match_date, start_time, duration_minutes,
      ...
      ORG_UID, 'active', CLUB_ID, '{}'::uuid[],
      current_date, '12:24'::time, 90,
      'tr_v2_D05_remove_promotes_waitlist', 2,
      ARRAY[SCOPE_GID]::uuid[],
      true, true, true, now()
    )
    RETURNING id INTO v_mid;
    ...
    PERFORM set_config(
      'request.jwt.claims',
      json_build_object('sub', LEO_UID::text, 'role', 'authenticated')::text,
      true
    );
    PERFORM public.rpc_match_accept_invite(v_mid);
    ...
    SELECT * INTO v_mp
    FROM public.match_participants
    WHERE public.match_participants.match_id = v_mid AND public.match_participants.user_id = LEO_UID
    ORDER BY created_at DESC
    LIMIT 1;
    PERFORM public.rpc_match_org_approve_participant(v_mp.id);
    PERFORM public.rpc_match_rebalance_roster(v_mid);
    ...
    FROM public.match_participants mp
    WHERE mp.match_id = v_mid
      AND mp.user_id = LEO_UID
      AND mp.removed_at IS NULL
      AND mp.status::text = 'confirmed';

    IF v_cnt = 1 THEN
      INSERT INTO _v2_results VALUES ('D05 Organizer remove promotes waiting list', true, 'ok', v_mid);
    ELSE
      INSERT INTO _v2_results VALUES ('D05 Organizer remove promotes waiting list', false, 'promoted_count='||v_cnt::text, v_mid);
    END IF;
  EXCEPTION WHEN OTHERS THEN
    INSERT INTO _v2_results VALUES ('D05 Organizer remove promotes waiting list', false, 'exception: '||SQLERRM, v_mid);
  END;
