...
assertIncludes(smsRenderer, 'Reply YES ${data.replyCode} or NO ${data.replyCode}.', 'SMS invitation reply commands')
assertIncludes(smsRenderer, 'Reply YES ${match.replyCode} or NO ${match.replyCode}.', 'SMS match invite reply commands')
...
assertIncludes(worker, 'async function resolveSmsJoinUrl(', 'queued SMS worker')
assertIncludes(worker, "responseUrl: d.channel === 'sms' ? await resolveSmsJoinUrl(supabase, matchId, 'respond') : null", 'queued SMS invitation link')
...
assertIncludes(worker, 'return joinPath ? new URL(joinPath, SMS_SITE_URL).toString() : null', 'queued SMS absolute URL')
