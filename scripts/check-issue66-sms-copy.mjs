...
assertIncludes(renderer, 'level_label?: string | null', 'InvitationSmsData.matchSummary')
assertIncludes(renderer, 'match_summary_sms?: string | null', 'InvitationSmsData.matchSummary')
assertIncludes(renderer, 'sportName?.trim().toLowerCase()', 'formatActivityLabel')
assertIncludes(renderer, 'formatMatchKind(data.matchSummary?.sport_name, data.matchSummary?.game_type, levelLabel)', 'renderInvitationSms')
assertIncludes(renderer, 'openingLine(data.recipientName, data.inviterDisplayName, matchKind, venueName, dateTime)', 'renderInvitationSms')
assertIncludes(renderer, "Reply YES ${data.replyCode} if you'd like to play, or NO ${data.replyCode} if not this time.", 'renderInvitationSms')
assertIncludes(renderer, 'sportName?: string | null', 'MatchSmsData')
assertIncludes(renderer, 'levelLabel?: string | null', 'MatchSmsData')
assertIncludes(renderer, 'matchSummarySms?: string | null', 'MatchSmsData')
assertIncludes(renderer, 'formatMatchKind(match.sportName, match.gameType, match.levelLabel)', 'renderMatchInviteSms')
assertIncludes(renderer, 'openingLine(match.recipientName, organizerName, matchKind, location, dateTime)', 'renderMatchInviteSms')
...
assertIncludes(worker, ".select('game_type, sport_id, match_date, start_time, venue_id, level, required_count')", 'enrichInvitationContext match query')
...
assertIncludes(worker, 'level_label: matchRow?.level', 'enrichInvitationContext level return')
assertIncludes(worker, 'match_summary_sms:', 'enrichInvitationContext SMS summary return')
...
assertIncludes(worker, 'levelLabel: ((payload.level_label as string)', 'buildMatchInfo levelLabel')
assertIncludes(worker, 'matchSummarySms: ((payload.match_summary_sms as string)', 'buildMatchInfo matchSummarySms')
